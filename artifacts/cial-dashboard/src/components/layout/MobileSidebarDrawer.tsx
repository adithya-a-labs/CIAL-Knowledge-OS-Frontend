...
import { X, Home, Bot, FileText, BookOpen, Box, ShieldCheck, HelpCircle, Users, BarChart2, Settings, Leaf } from 'lucide-react';
...
const ICON_MAP: Record<string, React.ComponentType<{ size?: number; className?: string }>> = {
  Home, Bot, FileText, BookOpen, Box, ShieldCheck, HelpCircle, Users, BarChart2, Settings
};
...
export default function MobileSidebarDrawer({ open, onClose }: MobileSidebarDrawerProps) {
  ...
  return (
    <div className="lg:hidden fixed inset-0 z-50 flex">
      ...
      {/* Drawer */}
      <aside
        className="relative w-64 bg-white flex flex-col h-full shadow-2xl animate-in slide-in-from-left duration-200"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-[#e2eedd]">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-[#f0f7ed] text-[#5a7a52]"
          ...
          {visibleItems.map((item) => {
            ...
            return (
              <Link
                ...
                `}
              >
                <IconComponent size={18} className={active ? 'text-white' : 'text-[#5a7a52]'} />
